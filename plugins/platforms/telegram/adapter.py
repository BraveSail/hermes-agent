...
def check_telegram_requirements() -> bool:
    ...
    try:
        ...
        InlineQueryResultArticle = getattr(_tg, "InlineQueryResultArticle", None)
        InputTextMessageContent = getattr(_tg, "InputTextMessageContent", None)
        ...
        TelegramBaseHandler = getattr(_ext, "BaseHandler", TelegramBaseHandler)
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _start_webhook_mode(self, webhook_url: str, *, is_reconnect: bool) -> None:
        ...
        await self._app.updater.start_webhook(
            ...
            secret_token=webhook_secret, allowed_updates=self._allowed_update_types(),
    ...
    async def _answer_guest_query(
        ...
        try:
            ...
            if utf16_len(text) > self.MAX_MESSAGE_LENGTH:
                # answerGuestQuery is one-shot: the client renders a single inline result, so the
                # first chunk must carry an explicit truncation notice instead of silently
                # dropping the tail.
    ...
    async def send(
        ...
        try:
            ...
            # Auto-fold deliberately stays on MarkdownV2 because expandable blockquotes are a Telegram
            # presentation feature, not generic rich markdown.
            if not auto_fold and self._should_attempt_rich(content, metadata=metadata):
                ...
                if rich_result is not None:
                    ...
                    return rich_result
            # Format and split the message. Auto-fold wraps each chunk independently so every Telegram
            # payload is valid expandable-quote MarkdownV2, including replies longer than 4096 UTF-16 units.
            formatted = self.format_message(content)
            chunk_limit = self.MAX_MESSAGE_LENGTH - 16 if auto_fold else self.MAX_MESSAGE_LENGTH
            while True:
                chunks = self.truncate_message(formatted, chunk_limit, len_fn=utf16_len)
                if len(chunks) > 1:
                ...
                if not auto_fold:
                    break
                folded_chunks = [self._wrap_expandable_blockquote(chunk) for chunk in chunks]
                overflow = max(
                    (utf16_len(chunk) - self.MAX_MESSAGE_LENGTH for chunk in folded_chunks), default=0)
                if overflow <= 0:
            ...
            if auto_fold:
                logger.info(
                    "[%s] auto-fold triggered: chat_type=%r len=%d chunks=%d", self.name,
                    (metadata or {}).get("chat_type"), len(content), len(chunks))
            message_ids = []
            thread_id = self._metadata_thread_id(metadata)
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Guest queries have no ordinary chat transport: a chat action would either leak into a
        # chat the bot is not a member of or be rejected outright.
    ...
    async def _cache_inbound_document(self, msg, event: MessageEvent, *, _dispatch_immediately: bool = False) -> bool:
    ...
    async def _handle_media_message(
        ...
        *,
        ...
        """
        msg = _message if _message is not None else (getattr(update, "message", None) or self._effective_update_message(update))
        if not msg:
            return
        if not self._is_user_authorized_from_message(msg):
        ...
        if _message is None and not self._should_process_message(msg):
            if self._should_observe_unmentioned_group_message(msg):
                _event = self._build_message_event(msg, self._media_message_type(msg), update_id=getattr(update, "update_id", None))
                if msg.caption:
                    _event.text = self._clean_bot_trigger_text(expand_link_entities(msg))
                await self._cache_observed_media(msg, _event)
                self._observe_unmentioned_group_message(msg, _event.message_type, update_id=getattr(update, "update_id", None), event=_event)
            return
        event = self._build_message_event(msg, self._media_message_type(msg), update_id=getattr(update, "update_id", None))
        ...
        if msg.voice:
            ...
            if await self._cache_inbound_av(msg, event, msg.video, "video file", "video", ".mp4", "video/mp4"):
                return
        elif msg.document and await self._cache_inbound_document(msg, event, _dispatch_immediately=_dispatch_immediately):
            return
        media_group_id = getattr(msg, "media_group_id", None)
