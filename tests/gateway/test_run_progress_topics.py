...
@pytest.mark.asyncio
async def test_run_agent_progress_is_suppressed_in_group_topics(monkeypatch, tmp_path):
    ...
    # Local fork policy: tool progress is DM-only.  Group/topic chats keep
    # typing metadata for the originating topic, but do not receive noisy
    # progress bubbles.
    assert adapter.sent == []
...
@pytest.mark.asyncio
async def test_run_agent_progress_edits_are_suppressed_in_group_topics(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_TOOL_PROGRESS_MODE", "all")
    ...
    assert result["final_response"] == "done"
    # Even adapters that support metadata-aware edits should not emit progress
    # edits in non-DM chats; local gateway policy keeps groups/channels quiet.
    assert adapter.sent == []
    assert adapter.edits == []
...
@pytest.mark.asyncio
async def test_run_agent_feishu_group_progress_is_suppressed(monkeypatch, tmp_path):
    """Local policy suppresses progress bubbles in non-DM Feishu topics."""
    monkeypatch.setenv("HERMES_TOOL_PROGRESS_MODE", "all")
