...
class TurnRunner:
    ...
    def _setup_stream_consumer(self, platform_key):
        ...
        # Local fork §2: Telegram groups/channels/guest contexts get the final reply only — no
        # token streaming. Other platforms keep their configurable streaming behaviour.
        live_surfaces_allowed = ctx._live_surfaces_allowed
        if not live_surfaces_allowed:
            want_stream_deltas = False
        want_interim_messages = ctx.interim_assistant_messages_enabled and live_surfaces_allowed
        # Local fork §3: forward structured reasoning deltas to the consumer only when streaming,
        # reasoning display is on for this platform, and the live-surface gate allows it.
        want_reasoning_stream = want_stream_deltas and bool(
        ...
        # Local fork §3: structured reasoning rides the same consumer but its own queue lane.
        # Bound only when the consumer exists AND reasoning streaming is wanted; the exposed
        # callback is also handed to the agent as ``reasoning_callback``.
        reasoning_delta_cb = None
        if stream_consumer is not None and want_reasoning_stream:
            def _reasoning_delta_handler(text: str) -> None:
                if ctx._run_still_current():
                    stream_consumer.on_reasoning_delta(text)
            reasoning_delta_cb = _reasoning_delta_handler
        ...
        return stream_consumer, stream_delta_cb, interim_assistant_cb, want_interim_messages, reasoning_delta_cb
    ...
    def _wire_turn_agent_callbacks(self, agent, turn_route, reasoning_config,
        ...
        ctx = self._ctx
        runner = self._runner
        # Local fork §2: Telegram groups/channels suppress every live surface, including the
        # tool-progress callback object itself. Other platforms keep upstream's always-attached
        # callback (its body gates each event class, and subagent-failure notices must fire even
        # with tool_progress/thinking off).
        agent.tool_progress_callback = (
            ctx.progress_callback if ctx._live_surfaces_allowed else None
        )
        ...
        # Local fork §3: structured reasoning deltas → GatewayStreamConsumer.on_reasoning_delta,
        # only when streaming + show_reasoning + live-surface gate are all on (else None).
        agent.reasoning_callback = reasoning_delta_cb
    ...
    def run_sync(self):
        ...
        stream_consumer, stream_delta_cb, interim_cb, want_interim, reasoning_delta_cb = (
