...
class GatewayRunner(
    ...
    def _thread_metadata_for_source(
        ...
        if getattr(source, "platform", None) == Platform.TELEGRAM:
            # Local fork (§4/§2): Telegram policy decisions — long non-DM auto-fold and the
            # DM-only live-surface gate — read ``chat_type`` off the outbound metadata, exactly
            # like BasePlatformAdapter's regular reply path provides it. The runner also sends
            # notices and busy acknowledgements directly, so leaving that key to the base
            # adapter alone silently bypasses those rules on every runner-owned send.
    ...
    class _RunAgentDisplay:
        ...
        # Local fork: Telegram DM-only live-surface gate (see LOCAL_FORK_MODIFICATIONS.md §2).
        _live_surfaces_allowed: Any = None
