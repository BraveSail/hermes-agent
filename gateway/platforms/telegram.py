...
def _render_table_block_for_telegram(table_block: list[str]) -> str:
    ...
    for index, row in enumerate(table_block[2:], start=1):
        ...
        # so we do not produce ****double-wrapped**** that breaks formatting.
        _h = heading.strip()
        ...
        for header, value in zip(headers, data_cells):
            _value_cmp = value.strip()
            if _value_cmp.startswith("**") and _value_cmp.endswith("**") and len(_value_cmp) > 4:
                _value_cmp = _value_cmp[2:-2]
            if not has_row_label_col and _value_cmp == _h:
                continue
