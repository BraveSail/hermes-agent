...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    if platform == "telegram":
        ...
        if chat_type:
            # Local fork: auto-fold and the DM-only live-surface policy both
            # need chat_type on every outbound path, exactly as the base
            # adapter's regular reply path provides it.
