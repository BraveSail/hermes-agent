...
class GatewayTurnMixin:
    ...
    def _hmwa_prepend_reasoning(self, agent_result, response, source, _intentional_silence):
        ...
        requires an explicit per-platform opt-in (scratch text, not final-answer content). Local fork
        §2: Telegram groups/channels/guest contexts never render the reasoning block."""
        from gateway.run import (
            _allows_live_chat_surfaces, _load_gateway_config, _platform_config_key,
        ...
        if _reasoning_style == "italic":
            ...
            # (each non-code segment wrapped in ``_…_``: MarkdownV2 emphasis cannot span blank
            # lines, and a ``*…*`` wrap cannot nest with the line's own ``**bold``), verbatim
            # code fences that stay paired across the line cap, separator line before the
            # answer. The raw body is passed (not the pre-truncated ``display_reasoning``) so
            # the renderer caps and counts the lines itself — re-capping an already-capped
            # body reported a bogus "1 more lines".
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        from gateway.run import (
            _allows_live_chat_surfaces, _gateway_platform_value, _has_platform_display_override,
        ...
        # Local fork (§2): Telegram groups/channels/guest contexts get final replies only —
        # live progress/interim/thinking/reasoning surfaces stay silent. Other platforms keep
        # upstream's configurable behaviour.
        is_webhook = source.platform == Platform.WEBHOOK
        _live_surfaces_allowed = _allows_live_chat_surfaces(source)
        tool_progress_enabled = (
            progress_mode not in {"off", "log"} and not is_webhook and _live_surfaces_allowed
        )
        ...
        return self._RunAgentDisplay(
            ...
            _live_surfaces_allowed=_live_surfaces_allowed,
