...
from gateway.stream_consumer_fences import (
...
# typing is driven by the seed frame; lazy re-seed measured 48s of dead air), _REASONING
# (structured reasoning delta; rides a dedicated lane so it never pollutes the answer
# ledger).  Tuples: (_COMMENTARY, text); (_TOOL_PROGRESS, line) native-bubble overlay;
# (_REASONING, text); (_FINAL_TEXT, text) authoritative final_response incl. post-stream
# augmentation, queued just before _DONE; (_FLUSH, threading.Event) barrier;
# (_APPROVAL_BOUNDARY, future, cancelled_flag).
_DONE = object()
_NEW_SEGMENT = object()
_COMMENTARY = object()
...
class GatewayStreamConsumer(StreamTransportMixin, StreamFallbackMixin, StreamThinkFilterMixin):
    ...
    def _reset_message_state(self) -> None:
        ...
        # Structured reasoning is kept SEPARATE from the answer ledger.  It is rendered
        # as a bounded prefix for live frames, while final-response reconciliation keeps
        # comparing only the assistant answer.  Cleared with the rest of the segment.
    ...
    def _display_payload(self, text: str) -> str:
        """Normalize like ``_send_or_edit`` output: reasoning-prefix strip + directive
        strip + fence close + strip.  The stripped reasoning prefix is presentation-only,
        so it must never enter the answer ledger (has_delivered_text /
        delivered_final_matches compare assistant answers, not live frames)."""
        text = self._strip_reasoning_prefix(text or "")
        return ensure_closed_code_fences(self._clean_for_display(text)).strip()
    ...
    def _reset_segment_state(self, *, preserve_no_edit: bool = False) -> None:
        if preserve_no_edit and self._message_id == "__no_edit__":
            return
        # Retain the segment's visible text so has_delivered_text still matches.  The
        # live reasoning prefix is presentation-only, so strip it before recording the
        # answer ledger entry.
        finalized = self._clean_for_display(
            self._strip_reasoning_prefix(self._last_sent_text)).strip()
        if finalized:
    ...
    def on_reasoning_delta(self, text: str) -> None:
        """Thread-safe: queue a structured reasoning delta on its own lane.

        Kept out of ``on_delta`` so reasoning never reaches ``_accumulated`` (the answer
        ledger) — it only feeds the live presentation prefix.
        """
        if text:
        ...
        try:
            while True:
                ...
                if self._should_edit(tick) and (
                    self._accumulated or self._reasoning_accumulated
    ...
    def _drain_queue(self) -> "_Tick":
        ...
        while True:
            ...
            elif kind is _REASONING:  # keep draining to batch reasoning deltas
                if item[1]:
                    self._reasoning_accumulated += item[1]
                    tick.reasoning_changed = True
    ...
    def _should_edit(self, tick: "_Tick") -> bool:
        ...
        if self._use_native_streaming:
            ...
            should_edit = bool(self._accumulated) or self._tool_progress_active or tick.reasoning_changed
        else:
    ...
    async def _push_update(self, tick: "_Tick") -> None:
        ...
        if tick.is_interim:
            if self._use_native_streaming:
                ...
                display_text += self.cfg.cursor
        # Structured reasoning rides the PRESENTATION payload only (bounded prefix +
        # assistant answer).  The answer ledgers stay reasoning-free: _display_payload
        # strips this prefix back off before any reconciliation.
        # The FINAL frame drops it (local fork): streaming already showed the reasoning
